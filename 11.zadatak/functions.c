...
hashPointer HTInitialization(int HTSize)
{
    hashPointer H=NULL;
    int i=0;
    H=(hashPointer)malloc(sizeof(hashTab));
    if(!H)
    {
    ...
    H->tabSize=HTSize;
    H->hashLists=(List*)malloc(sizeof(List)*H->tabSize);
    if(!H->hashLists)
    {
        perror("Couldn't allocate memory\n");
        return NULL;
...
int InsertListIntoHT(hashPointer hashT,List head)
{
    int i,key=0;
    List temp=head;
    while(temp->next)
    {
        temp=temp->next;
        for(i=0; i<strlen(temp->country); i++)
            if(i<5)
                key+=temp->country[i];
            else
                i=strlen(temp->country);
        InsertSortedCountriesIntoList(temp,hashT->hashLists[key]);
